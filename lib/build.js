...
async function build() {
    ...
    await render('landing', 'index.html', { activeProducts, recentPosts });
    ...
    for (const product of allProducts) {
        const slug = product.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').substring(0, 80);
        console.log(`   - Product: ${product.name}`);
        await render('product', `products/${slug}/index.html`, { 
            product, 
